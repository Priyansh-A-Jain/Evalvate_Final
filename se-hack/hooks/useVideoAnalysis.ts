...
function aggregateResults(results: VideoAnalysis[]): VideoAnalysis {
  ...
  return {
    ...
    faces_detected: Math.max(...results.map((r) => r.faces_detected ?? 0)),
    details: {
      shoulder_alignment: {
        angle_deg: avg(
  ...
  const captureAndBuffer = useCallback(async () => {
    ...
    try {
      ...
      if (ctx) {
        ...
        const res = await backendClient.post("/video/analyze-frame", {
          frame: dataUrl,
        });
      ...
      if (err.code === "ERR_NETWORK") {
        setError("Backend connection refused. Ensure the backend server is running.");
      }
    } finally {
