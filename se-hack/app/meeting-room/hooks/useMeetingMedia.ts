...
import { backendBaseUrl, backendWebSocketBaseUrl } from "@/lib/backend";
...
export function useMeetingMedia(
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        const res = await axios.post(`${backendBaseUrl}/video/analyze-frame`, {
          frame: dataUrl,
        });
  ...
  useEffect(() => {
    ...
    try {
      ws = new WebSocket(`${backendWebSocketBaseUrl}/voice/stream`);
      wsRef.current = ws;
