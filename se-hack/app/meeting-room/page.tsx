...
export default function MeetingRoomPage() {
  ...
  const submitResponse = async (payload: { answerText?: string; audioBase64?: string; audioMimeType?: string }) => {
    ...
    try {
      ...
      if (nextQuestion) {
        if (nextQuestion.suggested_delay_ms) {
          window.setTimeout(() => setIsQuestionAudioPlaying(Boolean(nextQuestion.audio_data_uri)), nextQuestion.suggested_delay_ms);
        } else {
          setIsQuestionAudioPlaying(Boolean(nextQuestion.audio_data_uri));
        }
      }
