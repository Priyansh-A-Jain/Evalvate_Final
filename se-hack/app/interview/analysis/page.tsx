...
function InterviewAnalysisInner() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setData(result);
        track("report_viewed", { interview_id: interviewId, role: result.interview.role });
      } catch (err: any) {
  ...
  async function handleShare() {
    ...
    setSharing(true);
    try {
      const { share_token } = await createShareToken(interviewId);
      const shareUrl = `${window.location.origin}/report/${share_token}`;
      await navigator.clipboard.writeText(shareUrl);
      setShareCopied(true);
      track("report_shared", { interview_id: interviewId });
  ...
  // Prefer the weighted multi-modal overall score computed by the scoring engine
  const overallScore = sessionAnalysis?.overall_score != null ? Math.round(sessionAnalysis.overall_score) : avgScore;
  const grade = sessionAnalysis?.grade ?? null;
  const componentScores = sessionAnalysis?.component_scores ?? null;
  const prosody = (sessionAnalysis?.prosody ?? null) as {
    pace_wpm?: number;
    filler_count?: number;
    total_words?: number;
    source?: string;
  ...
  // Compute radar data: weighted component scores when available, behavioral fallback otherwise
  ...
  const radarData = componentScores
    ? [
        { label: "Technical", value: Math.min(100, Math.max(0, (componentScores.technical ?? 5) * 10)), color: "#6366f1" },
        { label: "Communication", value: Math.min(100, Math.max(0, (componentScores.communication ?? 5) * 10)), color: "#10b981" },
        { label: "Eye Contact", value: Math.min(100, Math.max(0, (componentScores.eye_contact ?? 5) * 10)), color: "#06b6d4" },
        { label: "Emotion", value: Math.min(100, Math.max(0, (componentScores.emotion ?? 5) * 10)), color: "#f59e0b" },
        { label: "Structure", value: Math.min(100, Math.max(0, (componentScores.structure ?? 5) * 10)), color: "#8b5cf6" },
      ]
    : [
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-7 pb-12">
        ...
        <div className="flex flex-col items-end gap-3">
          <div className="text-right">
            <div className="flex items-baseline justify-end gap-2">
              {grade && (
                <span className="rounded-lg bg-[var(--accent-primary)]/10 px-2.5 py-1 text-lg font-extrabold text-[var(--accent-primary)]">
                  {grade}
                </span>
              )}
              <span className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-secondary)]">
                {overallScore}%
              </span>
            </div>
            <p className="text-sm text-[var(--text-tertiary)] font-medium mt-1">
              {componentScores ? "Weighted Overall Score" : "Overall Score"}
            </p>
          </div>
          <div className="flex gap-2 print:hidden">
            <button
              onClick={handleShare}
              disabled={sharing}
              className="flex items-center gap-1.5 rounded-xl border border-[var(--border-default)] bg-white/80 px-3.5 py-2 text-xs font-semibold text-[var(--text-secondary)] hover:bg-white hover:text-[var(--accent-primary)] transition-all disabled:opacity-50"
            >
              {shareCopied ? <CheckCircle2 className="w-3.5 h-3.5 text-[var(--accent-success)]" /> : <Share2 className="w-3.5 h-3.5" />}
              {shareCopied ? "Link copied!" : "Share Report"}
            </button>
            <button
              onClick={() => window.print()}
              className="flex items-center gap-1.5 rounded-xl bg-[var(--accent-primary)] px-3.5 py-2 text-xs font-semibold text-white hover:bg-[var(--accent-primary)]/90 transition-all"
            >
      ...
      {/* Background person warning */}
      {sessionAnalysis?.background_person_detected && (
        <div className="rounded-xl border border-[var(--accent-warning)]/30 bg-[var(--accent-warning)]/8 px-4 py-3 flex items-start gap-2.5">
          <AlertTriangle className="w-4 h-4 text-[var(--accent-warning)] mt-0.5 shrink-0" />
          <div>
            <p className="text-sm font-semibold text-[var(--text-primary)]">Additional person detected on camera</p>
            <p className="text-xs text-[var(--text-secondary)] mt-0.5">
              More than one face appeared in {Math.round((sessionAnalysis.multi_face_ratio ?? 0) * 100)}% of analyzed
              frames. In a proctored interview this would be flagged for review.
            </p>
          </div>
        </div>
      )}

      {/* Speech delivery metrics from prosody analysis */}
      {prosody && (prosody.pace_wpm != null || prosody.filler_count != null) && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {prosody.pace_wpm != null && (
            <StatCard
              icon={<Activity className="w-5 h-5" />}
              label="Speaking Pace"
              value={`${Math.round(prosody.pace_wpm)} WPM`}
              color={prosody.pace_wpm >= 120 && prosody.pace_wpm <= 160 ? "var(--accent-success)" : "var(--accent-warning)"}
            />
          )}
          {prosody.filler_count != null && (
            <StatCard
              ...
              label="Filler Words"
              value={`${prosody.filler_count}`}
              color={prosody.filler_count <= 5 ? "var(--accent-success)" : "var(--accent-warning)"}
            />
          )}
          {prosody.total_words != null && (
            <StatCard icon={<FileText className="w-5 h-5" />} label="Words Spoken" value={`${prosody.total_words}`} color="var(--accent-primary)" />
          )}
          {prosody.source === "hume" && prosody.top_emotions && prosody.top_emotions.length > 0 && (
            <StatCard
              icon={<Sparkles className="w-5 h-5" />}
              label="Top Vocal Emotion"
              value={prosody.top_emotions[0].name}
